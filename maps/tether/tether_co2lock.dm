...
obj/machinery/airlock_sensor/co2/process()
	if(on)
		...
		if(abs(pressure - previousPressure) > 0.1 || previousPressure == null || abs(co2 - previousco2) > 0.1 || previousco2 == null)
			...
			previousco2 = co2
			alert = (pressure < ONE_ATMOSPHERE*0.8) || (co2 > 0.5)
			update_icon()
...
//
/obj/machinery/embedded_controller/radio/airlock/co2
	var/tag_scrubber

/obj/machinery/embedded_controller/radio/airlock/co2/initialize()
	. = ..()
	program = new/datum/computer/file/embedded_program/airlock/co2(src)

//Advanced airlock controller for when you want a more versatile airlock controller - useful for turning simple access control rooms into airlocks
/obj/machinery/embedded_controller/radio/airlock/co2
	name = "Carbon Dixoide Lock Controller"

/obj/machinery/embedded_controller/radio/airlock/co2/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	var/data[0]

	data = list(
		"chamber_pressure" = program.memory["chamber_sensor_pressure"],
		"chamber_co2" = program.memory["chamber_sensor_co2"],
	...
	if (!ui)
		ui = new(user, src, ui_key, "co2_airlock_console.tmpl", name, 470, 290)
		ui.set_initial_data(data)
		ui.open()
		ui.set_auto_update(1)

/obj/machinery/embedded_controller/radio/airlock/co2/Topic(href, href_list)
	if(..())
		return
...
/datum/computer/file/embedded_program/airlock/co2
	var/tag_scrubber

/datum/computer/file/embedded_program/airlock/co2/New(var/obj/machinery/embedded_controller/M)
	..(M)
	memory["chamber_sensor_co2"] = 0
	memory["external_sensor_pressure"] = VIRGO3B_ONE_ATMOSPHERE
	memory["external_sensor_co2"] = VIRGO3B_MOL_CO2
	memory["internal_sensor_co2"] = 0
	memory["scrubber_status"] = "unknown"
	memory["target_co2"] = 0.25
	memory["secure"] = 1

	if (istype(M, /obj/machinery/embedded_controller/radio/airlock/co2))	//if our controller is an airlock controller than we can auto-init our tags
		var/obj/machinery/embedded_controller/radio/airlock/co2/controller = M
		tag_scrubber = controller.tag_scrubber ? controller.tag_scrubber : "[id_tag]_scrubber"

/datum/computer/file/embedded_program/airlock/co2/receive_signal(datum/signal/signal, receive_method, receive_param)
	var/receive_tag = signal.data["tag"]
	if(!receive_tag) return
	if(..()) return 1

	if(receive_tag==tag_chamber_sensor)
		memory["chamber_sensor_co2"] = text2num(signal.data["carbon_dioxide"])
		memory["chamber_sensor_pressure"] = text2num(signal.data["pressure"])

	else if(receive_tag==tag_exterior_sensor)
		memory["external_sensor_co2"] = text2num(signal.data["carbon_dioxide"])

	else if(receive_tag==tag_interior_sensor)
		memory["internal_sensor_co2"] = text2num(signal.data["carbon_dioxide"])
...
/datum/computer/file/embedded_program/airlock/co2/process()
	switch(state)
		if(STATE_IDLE)
		...
		if(STATE_PREPARE)
			if (check_doors_secured())
				if(target_state == TARGET_INOPEN)
					if(memory["chamber_sensor_co2"] > memory["target_co2"])
						state = STATE_CLEAN
		...
		if(STATE_CLEAN)
			if(!check_doors_secured())
				...
				stop_cycling()
			else if(memory["chamber_sensor_co2"] <= memory["target_co2"])
...
/datum/computer/file/embedded_program/airlock/co2/stop_cycling()
	state = STATE_IDLE
	target_state = TARGET_NONE
	signalPump(tag_airpump, 0)
	signalScrubber(tag_scrubber, 0)

/datum/computer/file/embedded_program/airlock/co2/proc/signalScrubber(var/tag, var/power)
	var/datum/signal/signal = new
	signal.data = list(
		"tag" = tag,
		"sigtype" = "command",
